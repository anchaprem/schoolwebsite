...
let srpValidation =   () => {
    ...
    if (fatherPH.value === "" || fatherPH.value.length !=13  || isNaN(fatherPH.value)  ) {
        firstName.style="border-color: red";
        firstnameError.innerHTML = "";
        lastName.style="border-color: red";
        lastNameError.innerHTML = "";
        ...
        fatherPhError.innerHTML = "Fill  this Field. Enter phone number with dailing code Number Cnic  & not allowed Characters ";
        return false;
    }
    if (userDob.value === "" ) {
        firstName.style="border-color: red";
...
submitError.innerHTML="Submitting Please Wait....";
 ...
 .then((userCredential) => {
    ...
    // ...
    firebase.database().ref(`crest/teachers/`+user.uid).child("ProfileData").set(data).then(() => {
